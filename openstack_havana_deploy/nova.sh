...
function controller_nova_setup() {
  if [ ${HA_ENABLE} -eq 1 ]; then {
        echo "controller_nova_setup: HA_ENABLE"
        keystone_ip=${VIP_OP}
        db_ip=${VIP_MySQL}
  ...
  setconf infile:$base_dir/conf/etc.nova/nova.conf outfile:/etc/nova/nova.conf \
    ...
    if [ ${HA_ENABLE} -eq 1 ]; then {
        target_file=/etc/nova/nova.conf
...
function compute_nova_setup() {
  if [ ${HA_ENABLE} -eq 1 ]; then {
        echo " compute_nova_setup: HA_ENABLE"
        controller_node_pub_ip=${VIP_PUBLIC}
        controller_node_ip=${VIP_OP}
  ...
  if [ ${vagrant_debug} -eq 1 ]; then {
     target_file=/etc/nova/nova-compute.conf
     Replace "libvirt_type" "libvirt_type=qemu" "${target_file}"
     target_file=/etc/nova/nova.conf
     Append "libvirt_type=qemu" ${target_file} 
  };fi
